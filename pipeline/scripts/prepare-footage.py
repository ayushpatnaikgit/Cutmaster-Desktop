...
Usage: python3 scripts/prepare-footage.py <camera.mov> [--fast]
(--fast encodes several times quicker, for a slightly bigger file.)
...
preset = 'veryfast' if '--fast' in sys.argv else 'medium'
...
subprocess.run(['ffmpeg', '-v', 'error', '-y', '-ss', str(start), '-t', str(dur), '-i', sys.argv[1], '-an',
                '-vf', 'format=yuv420p', '-c:v', 'libx264', '-preset', preset, '-crf', '17',
