...
crossfades a repeat in the middle to reach the full runtime, and ducks the bed
under Mayank's voice.
"""
import subprocess, sys

FPS = 25
INTRO, OUTRO = 6.0, 8.0
FOOTAGE = round((205.6 - 1.4) * FPS) / FPS
TOTAL = INTRO + FOOTAGE + OUTRO
XF = 4.0

name = sys.argv[1]
under_db = float(sys.argv[2]) if len(sys.argv) > 2 else -19
src = f'public/audio/{name}.mp3'
...
subprocess.run(['ffmpeg', '-v', 'error', '-y', '-i', src, '-filter_complex', graph, '-map', '[out]', '-ac', '2', '-c:a', 'pcm_s16le', 'public/audio/music_mix.wav'], check=True)
